...
struct StochasticRoundingRNGState {
    ...
    StochasticRoundingRNGState(const PhiloxXpuState& philox_state,
    ...
    constexpr uint64_t splitmix64_stateless(uint64_t index) noexcept {
    ...
    void init(const PhiloxXpuState& philox_state,
        ...
        state =
            splitmix64_stateless(s0 ^ s1) ^ splitmix64_stateless(salt_value);
...
static __half stochastic_rounding_scalar(float x, uint32_t random_value) {
    ...
    float assemble_float =
        sycl::bit_cast<float>(assembles) - sycl::bit_cast<float>(subtract);
    return sycl::vec<float, 1>(x + assemble_float)
        .convert<__half, sycl::rounding_mode::rtz>()[0];
}
...
inline void stochastic_rounding_vector(dst_t* output, const Vec4T<src_t>& value,
                                       StochasticRoundingRNGState& state) {
    value.store(output);
}

template <>
inline void stochastic_rounding_vector(at::Half* output,
    ...
    sycl::multi_ptr<sycl::half, sycl::access::address_space::global_space> mp(
        out_sycl);
    v.store(0, mp);
}
...
template <typename dst_t, typename src_t>
void nearest_rounding_vector(dst_t* output, const Vec4T<src_t>& value) {
    value.store(output);
}
